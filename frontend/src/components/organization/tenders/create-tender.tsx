...
function UploadFlow() {
  const router=useRouter(); const [file,setFile]=useState<File|null>(null); const [preview,setPreview]=useState(false); const [demo,setDemo]=useState(false);
  function submit(event:FormEvent){event.preventDefault();if(file||demo)setPreview(true)}
  return <section className={styles.flowPanel}><h2>Existing Tender Document</h2><form className={styles.formGrid} onSubmit={submit}><div className={`${styles.drop} ${styles.fieldWide}`}><strong>Drop a tender PDF here</strong><p>PDF only · selected locally · nothing is uploaded</p><input type="file" accept="application/pdf,.pdf" aria-label="Choose tender PDF" onChange={event=>{setFile(event.target.files?.[0]??null);setPreview(false)}}/>{file?<div className={styles.selectedFile}><span>{file.name} · {(file.size/1024/1024).toFixed(2)} MB</span><button type="button" className={styles.action} onClick={()=>setFile(null)}>Remove</button></div>:null}</div><Field label="Tender Title" value={demo?"IT Software RFP":undefined}/><Field label="Tender Reference" value={demo?"IIITB/IT/RFP/2026":undefined}/><Field label="Procurement Category" value={demo?"Information Technology":undefined}/><Field label="Organization" value={demo?"IIIT Bangalore":undefined}/><div className={styles.actions}><button className="button button--ghost" type="button" onClick={()=>{setDemo(true);setPreview(false)}}>Load Blueprint Preview</button><button className="button" type="submit">Continue to Preview</button></div></form>{preview?<div className={styles.pipeline}>{["Document Ingest","Text Extraction","Section Detection","Requirement Extraction","Blueprint Generation"].map((item,index)=><div className={styles.pipelineStep} key={item}><small>0{index+1}</small><strong>{item}</strong></div>)}</div>:null}{preview?<div className={styles.integration}>Local preview only. No PDF extraction or external analysis has occurred. <button className={styles.action} type="button" onClick={()=>router.push("/organization/tenders/it-software-rfp/blueprint?state=processing")}>Analyze Tender →</button></div>:null}</section>;
}
